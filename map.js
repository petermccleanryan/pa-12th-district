 ...
 *
 * Code inspired by:
 * Lukas Vonlanthen (2015), lukas.vonlanthen[at]cde.unibe.ch
...
var topology,
    ...
    carto = d3.cartogram()
      ...
      .properties(function(d) {
        if (!dataById[d.properties.NAME]) {
          console.log('ERROR: Entry "' + d.properties.NAME + '" was found in the Topojson but not in the data CSV. Please correct either of them.');
        }
...
d3.json('pa.json', function(topo) {
  topology = topo;

  // The mapped unit for communes: Communes
  geometries = topology.objects.cb_2015_pennsylvania_county_20m.geometries;

  // Read the data for the cartogram
  d3.csv("voters.csv", function(data) {
...
function init() {
  ...
  */// Scale it to fit nicely
  proj
      .center([-77.194527, 41.203323])
      .scale(6500).translate([width / 2, height / 2]);
...
function showTooltip(d, i) {
  ...
  // the left.
  var left = Math.min(mouse[0]+20, (mouse[1]+20));
  ...
  tooltip.classed("hidden", false)
    ...
    .html([
      ...
      'Registered Democrats in PA-12: ', formatNumber(getValue(d)),
    ].join(''));
}
